...
FacebookTokenStrategy.prototype.authenticate = function(req, options) {
  ...
  if(req.body){
	  accessToken = req.body.fb_access_token;
	  refreshToken = req.body.fb_refresh_token;
  }
  
  accessToken = accessToken || req.query.fb_access_token;
  refreshToken = refreshToken || req.query.fb_refresh_token;
...
FacebookTokenStrategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.getProtectedResource(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      profile.age_range = json.age_range;
